...
for i in xrange(max(num_split, len(split_tp))):
    ...
    open(os.path.join(out_path, '{}_mvrgb_train_split_{}.txt'.format(dataset, i+1)), 'w').writelines(lists[0][0])
    open(os.path.join(out_path, '{}_mvrgb_val_split_{}.txt'.format(dataset, i+1)), 'w').writelines(lists[0][1])
    open(os.path.join(out_path, '{}_mvflow_train_split_{}.txt'.format(dataset, i+1)), 'w').writelines(lists[1][0])
    open(os.path.join(out_path, '{}_mvflow_val_split_{}.txt'.format(dataset, i+1)), 'w').writelines(lists[1][1])
